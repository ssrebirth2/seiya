...
export default function HeroBonds({ heroId }: HeroBondsProps) {
  ...
  const renderSkillDetails = (skill: any): React.ReactElement => {
    ...
    const mainDescription =
      desList.length > 0 ? applySkillValues(getT(desList[0].des), desList[0].value ?? 0, valuesMap) : ''
    ...
    return (
      <div
        key={skill.skillid}
        className="mt-6 rounded-xl border border-panel-border bg-panel p-5 shadow-lg backdrop-blur-md transition-all hover:shadow-xl"
      >
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div
      className="rounded-xl border border-panel-border bg-panel p-5 shadow-md backdrop-blur-md"
    >
