...
function Pill({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={`text-xs px-2 py-1 rounded-full border border-[var(--panel-border)] bg-[var(--panel)] ${className}`}
    >
      {children}
    </span>
  )
  ...
  return (
    <div className="flex items-center justify-between gap-2 min-w-0">
      <div className="flex items-center gap-2 min-w-0">
        <div className="w-8 h-8 rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] overflow-hidden">
        ...
        <div className="min-w-0">
          <div className="text-xs font-medium truncate" title={`${baseName} (ID ${id})`}>
            <span>{baseName}</span>{' '}
            <span className="text-[11px] text-[var(--text-muted)]">(ID {id})</span>
          </div>
          {showQtyLine ? <div className="text-[11px] text-[var(--text-muted)]">x{qty || 0}</div> : null}
        </div>
      </div>
...
export default function ItemsTable({
  ...
  return (
    <div className="border border-[var(--panel-border)] rounded-xl overflow-hidden bg-[var(--panel)]">
        ...
        <tbody>
          {visibleItems.map((it) => {
            ...
            return (
              <tr
  key={it.id}
  className="border-b border-[var(--panel-border)] last:border-b-0 hover:bg-[var(--panel-hover)] transition-colors"
>
  <td className="p-4">
    <div className="grid grid-cols-12 gap-6 items-start">
      ...
      <div className="col-span-12 sm:col-span-10">
        <div className="flex flex-col gap-3">
          ...
          {/* DESCRIPTION */}
          <div
            className="text-sm text-[var(--text-muted)] leading-snug"
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {it.isRare && (
              <Pill className="bg-yellow-500/20 border-yellow-500/30 text-yellow-600">
                {finalGetT('Rare')}
              </Pill>
            )}
          </div>
          ...
          {hasBox && (
            <div className="mt-4">
              <div className="text-xs font-semibold mb-3 uppercase tracking-wide text-[var(--text-muted)]">
                {finalGetT('Box') || 'Box'}
              </div>
                ...
                <div className="flex gap-4 min-w-max pb-2">
                  ...
                  {boxShow.map((x, idx) => {
                    ...
                    return (
                      <div
  key={`box-show-${id}-${idx}`}
  className="w-20 shrink-0 rounded-2xl border border-[var(--panel-border)] bg-[var(--panel)] p-4 hover:bg-[var(--panel-hover)] transition-all duration-200"
>
  ...
  className="relative w-20 shrink-0 rounded-2xl border border-[var(--panel-border)] bg-[var(--panel)] p-4 hover:bg-[var(--panel-hover)] transition-all duration-200"
>
  <div className="flex flex-col items-center gap-3">
    ...
    <div className="text-[13px] font-medium text-center leading-snug px-2">
      ...
      {loadingMoreDetails && (
        <div className="px-4 py-3 text-xs text-[var(--text-muted)] border-t border-[var(--panel-border)] text-center">
          {finalGetT('Loading more details...') || 'Loading more details...'}
        </div>
      )}

      {showLoadMore && (
        <div ref={loadMoreRef} className="p-8 text-center border-t border-[var(--panel-border)]">
          <button
            className="px-6 py-3 text-sm rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] hover:bg-[var(--panel-hover)] transition-colors font-medium"
      ...
      {!showLoadMore && items.length > 0 && (
        <div className="p-6 text-center border-t border-[var(--panel-border)] bg-[var(--panel-hover)]">
