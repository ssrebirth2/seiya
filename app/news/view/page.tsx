...
export default function NewsDetailPage() {
  ...
  if (error) {
    return (
      <div className="panel">
        <Link href="/news" className="text-sm underline">← Voltar</Link>
  ...
  return (
    <div className="panel">
      ...
      <div className="text-sm text-muted mt-1">
        Autor: {post.author ?? 'Desconhecido'} · Publicado: {post.published_at ?? 'Desconhecido'}
      </div>

      <div
        className="wechat-content mt-6"
        dangerouslySetInnerHTML={{ __html: post.content_html ?? '<p>Sem conteúdo</p>' }}
      />
    </div>
  )
}
